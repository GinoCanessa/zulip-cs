...
- **Consistency** — does this change follow the patterns already
  ...
  `dotnet build zulip-cs.sln` as the canonical build).
...
1. **Target** *(required)* — where to write the analysis. One of:
   - A **full path** (absolute or repo-relative) to a `.md` file. Used
     ...
     `C:\ai\git\zulip-cs\scratch\0501-04\analysis.md`.
...
3. **Pre-flight.**
   ...
   - Confirm the build/test commands referenced by the repo (e.g.,
     `dotnet build zulip-cs.sln`, `dotnet test zulip-cs.sln`)
...
- {Specific commands. E.g.,
  `dotnet test zulip-cs.sln --filter FullyQualifiedName~Foo`}
...
- **Honor repo conventions and stored memories.** Use them as the
  ...
  empty collections, `dotnet build zulip-cs.sln` and
  `dotnet test zulip-cs.sln` as the canonical build/test

...
4. **Final verification.** When the loop exits because all phases are
   ...
   default, `dotnet test zulip-cs.sln`) once more end-to-end.
...
- **Honor repo conventions and stored memories** — explicit C# types,
  `[]` for empty collections, `dotnet build zulip-cs.sln` and
  `dotnet test zulip-cs.sln` as the canonical build/test commands, and any
